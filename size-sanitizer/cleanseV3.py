...
def readJson(filename):
	...
	print("Initial file size: ", len(data)) 
	return data;
...
def debugPrint(currentData, fileName, matchStr, filterStr):
	...
	noOfClassified = len(currentData.index)
...
print("FIRST GROUP ", len(data_EUUK.index))
...
print("SECOND GROUP ", len(data_EUUKV2.index))
#Formatting step
...
print("THIRD GROUP ", len(data_Youth.index))
...
print("FOURTH GROUP ", len(data_NewBorn.index))
...
changeTracker.append({"matchStr": 'unclassified', "filterStr": 'na', "count":len(data_unclassified.index)});
